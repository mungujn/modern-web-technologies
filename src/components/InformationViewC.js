...
import {Paper, Typography, Avatar, Grid, ListItem, ListItemText, ListItemAvatar, Button} from "material-ui";
...
class InformationViewC extends Component {
    ...
    updateSupply(node){
        firebase.readData(node).then((snapshot) => {
            let x = snapshot.val();
            if(x !== null){
                this.props.setToolbarTitle(x.health_center_name+' Health Center');
                this.setState({
    ...
    updatePrediction(node){
        firebase.readData(node).then((snapshot) => {
            ...
            if(x !== null){
                this.props.setToolbarTitle(x.health_center_name+' Health Center');
                let object = {
                    "tape_measures": x.items.tape_measures,
                    "fetoscopes": x.items.fetoscopes,
                    "blood_pressure_machines": x.items.blood_pressure_machines,
                    "weighing_scales": x.items.weighing_scales,
                    "watches": x.items.watches,
                    "thermometers": x.items.thermometers,
                    "catheters": x.items.catheters,
                    "surgical_gloves": x.items.surgical_gloves,
                    "umbilical_cord_clamps": x.items.umbilical_cord_clamps,
                    "resuscitation_units": x.items.resuscitation_units,
                    "surgical_blades": x.items.surgical_blades,
                    "baby_weighing_scales": x.items.baby_weighing_scales,
                    "suturing_kits": x.items.suturing_kits,
                    "cotton_wool": x.items.cotton_wool,
                    "cannulas": x.items.cannulas,
                    "oxytocin": x.items.oxytocin,
                    "urine_testing_kits": x.items.urine_testing_kits,
                    "hiv_testing_kits": x.items.hiv_testing_kits,
                    "blood_grouping_kits": x.items.blood_grouping_kits,
                    "blood_for_transfusion": x.items.blood_for_transfusion
                };
                this.setState({
    ...
    updateRequest(node){
        firebase.readData(node).then((snapshot) => {
            let x = snapshot.val();
            if(x !== null){
                this.props.setToolbarTitle(x.health_center_name+' Health Center Supply Request');
                this.setState({
                    health_center_name: x.health_center_name + " Health Center",
                    items: x.items,
                    sub_title: 'Supplies needed for optimal operation this month'
                });
            }
        });
    }
    ...
    generateItem(name, value){
        return <ListItem button>
            ...
            <ListItemAvatar>
                <Avatar style={this.getStyle(name, value)}>
                    {value}
                </Avatar>
            </ListItemAvatar>
        </ListItem>
        ...
        return (
            <div>
                <Paper elevation={5} style={style.paper}>
                    <Grid container>
                        <Grid item>
                            <Avatar style={style.avatar_style}>{this.props.title[0]}</Avatar>
                        </Grid>
                        <Grid item>
